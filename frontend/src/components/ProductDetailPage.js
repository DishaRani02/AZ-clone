...
function ProductDetailPage({ product, addToCart, setShowDetail, toggleWishlist, isWishlisted }) {
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <img
            src={product.image}
            alt={product.name}
            style={{ width: '100%', maxHeight: '400px', objectFit: 'contain', border: '1px solid #f0f0f0', borderRadius: '8px', padding: '20px' }}
          />
        </div>
        ...
        <div>
          ...
          <div style={{ color: '#FF9900', fontSize: '18px', marginBottom: '10px' }}>
            ★★★★☆
            <span style={{ color: '#666', fontSize: '14px', marginLeft: '8px' }}>(1,243 ratings)</span>
          </div>
          ...
          <div style={{ marginBottom: '15px' }}>
            ...
            <div style={{ color: '#B12704', fontWeight: 'bold', fontSize: '28px' }}>₹{product.price}</div>
            <span style={{ color: '#007600', fontSize: '14px' }}>20% off | Free Delivery</span>
          </div>

          <hr style={{ border: '1px solid #f0f0f0', margin: '15px 0' }} />

          <p style={{ color: '#007600', fontWeight: 'bold', fontSize: '16px' }}>In Stock ✓</p>

          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px' }}>
            <span style={{ fontWeight: 'bold' }}>Quantity:</span>
            <button onClick={() => setQuantity(q => Math.max(1, q - 1))}
              style={{ padding: '4px 12px', fontSize: '18px', cursor: 'pointer', borderRadius: '4px', border: '1px solid #ddd' }}>-</button>
          ...
          <button onClick={handleAddToCart}
            style={{
              ...
              color: added ? 'white' : 'black', transition: 'background-color 0.3s'
            }}>
          ...
          <button
            ...
            style={{
              width: '100%', padding: '12px',
              backgroundColor: isWishlisted && isWishlisted(product.id) ? '#ff4444' : 'white',
              border: '1px solid #ddd', borderRadius: '20px', cursor: 'pointer',
              fontWeight: 'bold', fontSize: '16px', marginBottom: '10px',
              color: isWishlisted && isWishlisted(product.id) ? 'white' : 'black'
            }}>
            {isWishlisted && isWishlisted(product.id) ? '❤️ Wishlisted' : '🤍 Add to Wishlist'}
          </button>
